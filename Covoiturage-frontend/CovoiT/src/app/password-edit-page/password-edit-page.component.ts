  ...
  onSubmit() {
    if (this.form.valid) {
      if (this.form.value.password !== this.form.value.confirmPassword) {
        this.snackBar.open('Non concordance des mots de passe', 'Refaire', {
          duration: 3000
        });
        return;
      }
      ...
      this.apiService.updatePassword(this.form.value).subscribe(
        ...
        (error: any) => {
          this.snackBar.open(error.error, 'Refaire', {
            duration: 3000
          });
        }
      );
      ...
      this.snackBar.open('Veuillez remplir le formulaire', 'Refaire', {
        duration: 3000
      });
    }
  }
